        ...
        <div className="space-y-6">
          ...
          <div className="bg-[#0A0A0A] border border-[#333] rounded-xl p-6 space-y-6">
            <div className="flex items-center justify-between">
              ...
              <label className="relative cursor-pointer flex items-center gap-2 text-sm font-medium text-primary hover:text-primary/80 transition-colors">
                {loading ? (
                  <>
                    ...
                    <input
                      ...
                      onChange={async (e) => {
                        if (e.target.files?.length) {
                          ...
                          setLoading(true);
                          const { uploadImage } =
                            await import("@/app/admin/upload/actions");
                          const res = await uploadImage(formData);
                          setLoading(false);

                          if (res?.success && res.url) {
                            handleChange("cover_image", res.url);
